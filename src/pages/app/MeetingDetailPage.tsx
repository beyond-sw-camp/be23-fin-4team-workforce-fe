...
export default function MeetingDetailPage() {
  ...
  const updateMut = useMutation({
    mutationFn: (body: { scheduledAt?: string; agenda?: string }) =>
    ...
    onError: (error: any) =>
      message.error(error?.response?.data?.message ?? '면담 정보 수정에 실패했습니다.'),
  });

  const memberReactionMut = useMutation({
    mutationFn: (reaction: Reaction) =>
      meetingApi.recordMemberReaction(meetingId, { memberReaction: reaction }),
    onSuccess: () => {
      message.success('구성원 반응을 저장했습니다.');
      qc.invalidateQueries({ queryKey: ['meetings', meetingId] });
    },
    onError: (error: any) =>
      message.error(error?.response?.data?.message ?? '구성원 반응 저장에 실패했습니다.'),
  });
  ...
  const createActionMut = useMutation({
    ...
    onError: (error: any) =>
      message.error(error?.response?.data?.message ?? '후속 액션 추가에 실패했습니다.'),
  });
  ...
  const completeActionMut = useMutation({
    ...
    onError: (error: any) =>
      message.error(error?.response?.data?.message ?? '후속 액션 완료 처리에 실패했습니다.'),
  });
  ...
  const rateActionMut = useMutation({
    ...
    onError: (error: any) =>
      message.error(error?.response?.data?.message ?? '후속 액션 평가 저장에 실패했습니다.'),
  });
    ...
    <div className="tw-mx-auto tw-max-w-[900px] tw-p-6">
          ...
          <div className="tw-flex tw-flex-wrap tw-items-center tw-justify-between tw-gap-3">
            <div>
              ...
              <div className="tw-mt-1 tw-text-sm tw-text-slate-600">
                이 면담은 평가 결과 공개 후 자동 생성된 피드백 면담입니다. 결과 리뷰와 다음 액션
                정리에 집중해 주세요.
              </div>
            </div>
      ...
      <Card className="tw-mb-5">
        ...
        >
          ...
          <Descriptions.Item label="아젠다" span={2}>
            {meeting.agenda || '-'}
          </Descriptions.Item>
          {meeting.relatedSeasonId && (
            <Descriptions.Item label="연결 시즌">
              <Tag color="blue">{meeting.relatedSeasonId.slice(0, 8)}…</Tag>
            </Descriptions.Item>
          )}
          {isCompleted && (
            <Descriptions.Item label="완료 시각">
        ...
        <Card title="면담 완료 처리" className="tw-mb-5">
          ...
          >
            ...
            <Form.Item name="privateMemo" label="비공개 메모">
              <Input.TextArea
                rows={2}
                placeholder="매니저만 볼 수 있는 메모가 있으면 남겨 주세요."
              />
            </Form.Item>
      ...
      {!isCompleted && isMember ? (
        <Card className="tw-mb-5 tw-rounded-2xl tw-border tw-border-slate-200/90 tw-bg-slate-50/60">
          ...
          <div className="tw-mt-1 tw-text-sm tw-text-slate-500">
            면담 완료 처리는 상사가 진행합니다. 완료 후에는 이 화면에서 면담 기록과 후속 액션을
            확인할 수 있습니다.
          </div>
        </Card>
      ) : null}
        ...
        <>
          <Card title="면담 기록" className="tw-mb-5">
            <Typography.Paragraph>
              {meeting.memo || <Typography.Text type="secondary">기록 없음</Typography.Text>}
            </Typography.Paragraph>
            {meeting.managerReaction && (
              <div className="tw-mt-3">
                <Typography.Text type="secondary" className="tw-mr-2">
                  매니저 반응:
                </Typography.Text>
                {reactionConfig.find((item) => item.value === meeting.managerReaction)?.label ??
                  meeting.managerReaction}
              </div>
            )}
          </Card>
            ...
            {isMember ? (
              <>
                ...
                <ReactionPicker
                  ...
                  onChange={(value) => memberReactionMut.mutate(value)}
              ...
              <Typography.Text type="secondary">
                {meeting.memberReaction
                  ? (reactionConfig.find((item) => item.value === meeting.memberReaction)?.label ??
                    meeting.memberReaction)
                ...
                <List.Item.Meta
                  ...
                  title={
                    <Space>
                      ...
                      <Tag color={actionStatusColor[action.status as ActionStatus]}>
                        {actionStatusLabel[action.status as ActionStatus]}
                      </Tag>
                    </Space>
                  }
                  description={
                    <Space split={<Divider type="vertical" />} size={0}>
                      ...
                      {action.tlRating && (
                        <Tag
                          color={ratingConfig.find((item) => item.value === action.tlRating)?.color}
                        >
                          {ratingConfig.find((item) => item.value === action.tlRating)?.label}
                        </Tag>
                      )}
                    </Space>
                  }
                ...
                {action.status === 'COMPLETED' && isManager && (
                  <div className="tw-ml-4">
                    <TlRatingPicker
                      ...
                      onChange={(value) =>
                        rateActionMut.mutate({ actionId: action.meetingActionId, tlRating: value })
                      }
                    />
                  </div>
                )}
          ...
          <>
            ...
            >
              <Form.Item
                name="content"
                label="내용"
                rules={[{ required: true, message: '액션 내용을 입력해 주세요.' }]}
              >
      ...
      >
        ...
        >
          <Form.Item
            name="scheduledAt"
            label="일정"
            rules={[{ required: true, message: '면담 일정을 선택해 주세요.' }]}
          >
            <DatePicker
      ...
      {isCompleted ? (
        <Form form={completeForm} preserve={false} className="tw-hidden" aria-hidden />
      ) : null}
      {!showActionForm ? (
        <Form form={actionForm} preserve={false} className="tw-hidden" aria-hidden />
      ) : null}
    </div>
  );
}
          ...
          <Button
            ...
            style={
              value === reaction.value
                ? { background: reaction.color, borderColor: reaction.color }
                : {}
            }
            icon={reaction.icon}
  ...
  return (
    <Space>
      {ratingConfig.map((rating) => (
        ...
        >
          {value === rating.value ? (
            <StarFilled className="tw-mr-1" />
          ) : (
            <StarOutlined className="tw-mr-1" />
          )}
          {rating.label}
        </Tag.CheckableTag>
      ))}
