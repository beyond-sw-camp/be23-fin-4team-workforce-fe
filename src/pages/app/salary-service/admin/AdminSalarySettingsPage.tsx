...
export function SalaryTab({
  ...
  const createM = useMutation({
    ...
    onSuccess: ({ grantTotal, grantFailed }) => {
      if (grantTotal === 0) {
        ...
        message.warning(
          `급여는 등록됐지만 수당 ${grantFailed}/${grantTotal}건 부여 실패. [수당 관리] 에서 확인해주세요.`,
        );
      }
      setOpen(false);
      form.resetFields();
  ...
  const policyOptions = useMemo(
    () =>
      (policiesQ.data ?? []).map((p) => ({ value: p.salaryPolicyId!, label: p.policyName ?? '' })),
    [policiesQ.data],
  );
  ...
  const cols = useMemo<ColumnsType<Salary>>(
    () => [
      ...
      { title: '직책', dataIndex: 'jobTitleName', key: 'jobTitleName', width: 100 },
      {
        ...
        align: 'center',
        render: (v) =>
          v != null ? `${v}명` : <Typography.Text type="secondary">-</Typography.Text>,
      },
      {
        title: '적용 기간',
        key: 'eff',
        ...
        <>
          ...
          <Space wrap className="tw-mb-3">
            ...
            <Space size={6} className="tw-pl-2 tw-border-l tw-border-slate-200">
              <Switch size="small" checked={onlyNewHires} onChange={setOnlyNewHires} />
      ...
      >
        <div className="tw-px-5 tw-py-4">
          <Form<SalaryFormValues>
            ...
            layout="vertical"
            onFinish={(v) =>
              editing?.salaryId ? updateM.mutate({ id: editing.salaryId, v }) : createM.mutate(v)
            }
            onValuesChange={(changed, all) => {
              // 정책 변경 시 호봉제/연봉제 전환에 따라 step/baseSalary 초기화
              if ('salaryPolicyId' in changed) {
                const next = (policiesQ.data ?? []).find(
                  (p) => p.salaryPolicyId === changed.salaryPolicyId,
                );
          ...
          >
            ...
            {activePolicies.length === 1 && !editing ? (
              <>
                ...
                <Alert
                  ...
                  message={
                    <Typography.Text className="!tw-text-sm">
                      적용 급여 정책:{' '}
                      <Typography.Text strong>{activePolicies[0]?.policyName}</Typography.Text>
              ...
              >
                <Select
                  ...
                  placeholder="정책을 선택하면 호봉제/연봉제에 따라 입력 필드가 바뀝니다"
            ...
            <Form.Item
              noStyle
              shouldUpdate={(p, c) => p.salaryPolicyId !== c.salaryPolicyId || p.step !== c.step}
            >
              {({ getFieldValue }) => {
                ...
                return (
                  <Form.Item
                    ...
                    name="baseSalary"
                    rules={[
                      { required: true, message: '기본급을 입력하세요.' },
                      { type: 'number', min: 0 },
                    ]}
...
function SalaryPolicyTab() {
  ...
  const buildPayload = (v: PolicyFormValues) => ({
    ...
    fixedOvertimeMinutes:
      v.wageSystemType === 'NON_COMPREHENSIVE' ? 0 : (v.fixedOvertimeMinutes ?? 0),
    // 급여 정산 기간은 1일~말일 고정
  ...
  const updateM = useMutation({
    mutationFn: ({ id, v }: { id: string; v: PolicyFormValues }) =>
      salaryApi.salaryPolicy.update(id, buildPayload(v)),
    onSuccess: () => {
      message.success('수정 완료');
      setOpen(false);
      setEditing(null);
  ...
  const cols = useMemo<ColumnsType<SalaryPolicy>>(
    () => [
      ...
      { title: '지급일', dataIndex: 'payDay', key: 'payDay', width: 80, render: (v) => `${v}일` },
      {
        ...
        width: 120,
        render: (v) => (
          <Tag color={v === 'BEFORE' ? 'geekblue' : v === 'AFTER' ? 'purple' : 'default'}>
            {PAY_DAY_SHIFT_KO[v ?? 'BEFORE'] ?? v ?? '—'}
          </Tag>
        ),
      },
      {
        ...
        width: 110,
        render: (v) =>
          v === 'Y' ? <Tag color="geekblue">호봉제</Tag> : <Tag color="default">연봉협상제</Tag>,
      },
      {
        title: '임금제',
        dataIndex: 'wageSystemType',
        key: 'wageSystemType',
        width: 100,
        render: (v) => (
          <Tag color={v === 'COMPREHENSIVE' ? 'orange' : 'blue'}>{WAGE_SYS_KO[v] ?? v}</Tag>
        ),
      },
      {
        title: '월 소정근로시간',
  ...
  return (
    <>
      ...
      <AppDoubleActionModal
        ...
        confirmText={editing ? '수정' : '등록'}
        cancelText="취소"
        title={editing ? '급여 정책 수정' : '급여 정책 등록'}
        destroyOnHidden
        width={760}
      >
        <div className="tw-px-5 tw-py-4">
          <Form<PolicyFormValues>
            ...
            layout="vertical"
            onFinish={(v) =>
              editing?.salaryPolicyId
                ? updateM.mutate({ id: editing.salaryPolicyId, v })
                : createM.mutate(v)
            }
          >
            {/* 1행: 정책명 + 지급일 */}
            ...
            <Row gutter={12}>
              ...
              <Col span={8}>
                <Form.Item label="임금제 유형" name="wageSystemType" rules={[{ required: true }]}>
                  <Select
                    options={[
                      { value: 'COMPREHENSIVE', label: '포괄임금제' },
                      { value: 'NON_COMPREHENSIVE', label: '비포괄임금제' },
                    ]}
                  />
                </Form.Item>
              </Col>
              <Col span={6}>
                <Form.Item noStyle shouldUpdate={(p, c) => p.wageSystemType !== c.wageSystemType}>
                  {({ getFieldValue }) =>
                    getFieldValue('wageSystemType') === 'COMPREHENSIVE' ? (
              ...
              <Select
                ...
                options={[
                  ...
                  },
                  {
                    ...
                    label: (
                      <div className="tw-py-1">
                        <div className="tw-text-sm tw-text-slate-900">
                          월 소정근로일 (간이 22일)
                        </div>
                        <div className="tw-text-xs tw-text-slate-500">
                          시급제 회사용 - 영업일 기준
            ...
            <Alert
              ...
              message={
                <span className="tw-text-xs">
                  정산 기간 매월 1일 ~ 말일 고정 · 시급 환산 분모 209h · 통상임금 표준 30일 ·
                  연장근무 단위는 [연장근로 정책]에서 별도 관리
                </span>
              }
              className="!tw-py-2"
            />
...
function TaxRateTab({ readOnly = false }: { readOnly?: boolean } = {}) {
  ...
  const taxTypeOpts = Object.entries(TAX_TYPE_KO).map(([value, label]) => ({
    value,
    label: `${label} (${value})`,
  }));
    ...
    const baseCols: ColumnsType<TaxRate> = [
      {
        ...
        dataIndex: 'taxType',
        key: 'taxType',
        render: (v) => <Tag>{TAX_TYPE_KO[v] ?? v}</Tag>,
      },
      ...
      >
        <div className="tw-px-5 tw-py-4">
          <Form<TaxFormValues>
            ...
            layout="vertical"
            onFinish={(v) =>
              editing?.taxRateId ? updateM.mutate({ id: editing.taxRateId, v }) : createM.mutate(v)
            }
          >
            <Form.Item label="세금 유형" name="taxType" rules={[{ required: true }]}>
              <Select options={taxTypeOpts} />
            </Form.Item>
            <Form.Item label="적용 연도" name="applyYear" rules={[{ required: true }]}>
              <InputNumber min={2000} max={2099} style={{ width: '100%' }} />
            </Form.Item>
            <Form.Item
              label="근로자 부담률 (소수, 예: 0.045)"
              name="rate"
              rules={[{ required: true }]}
            >
              <InputNumber min={0} max={1} step={0.001} style={{ width: '100%' }} />
            </Form.Item>
            <Form.Item label="회사 부담률 (소수, 선택)" name="employerRate">
              <InputNumber min={0} max={1} step={0.001} style={{ width: '100%' }} />
            </Form.Item>
...
function SalaryItemTemplateTab() {
  ...
  const updateM = useMutation({
    mutationFn: ({ id, v }: { id: string; v: TemplateFormValues }) =>
      salaryApi.salaryItemTemplate.update(id, v),
    onSuccess: () => {
      message.success('수정 완료');
      setOpen(false);
      setEditing(null);
  ...
  const cols = useMemo<ColumnsType<SalaryItemTemplate>>(
    () => [
      ...
      },
      {
        ...
        key: 'itemType',
        width: 100,
        render: (v) => <Tag color={v === 'EARNING' ? 'green' : 'red'}>{ITEM_TYPE_KO[v] ?? v}</Tag>,
      },
      {
        title: '과세',
      ...
      >
        <div className="tw-px-5 tw-py-4">
          <Form<TemplateFormValues>
            ...
            layout="vertical"
            onFinish={(v) =>
              editing?.salaryItemTemplateId
                ? updateM.mutate({ id: editing.salaryItemTemplateId, v })
                : createM.mutate(v)
            }
          >
...
export function OvertimeUsageTab() {
  ...
  const effectiveDate = periodMode === 'WEEK' ? baseDate.endOf('week') : baseDate.endOf('month');
    ...
    () => [
      ...
      },
      {
        ...
        render: (v: number | null) => {
          ...
          if (v >= 100)
            return (
              <Tag color="red" className="!tw-text-sm">
                {v.toFixed(1)}%
              </Tag>
            );
          if (v >= 80)
            return (
              <Tag color="orange" className="!tw-text-sm">
                {v.toFixed(1)}%
              </Tag>
            );
  ...
  const bucketCols = useMemo<ColumnsType<OvertimeUsage>>(() => {
    ...
    return sample.map((b) => ({
      ...
      render: (_: unknown, row: OvertimeUsage) => {
        ...
        // 초과근무 있으면 노랑/주황 배경
        const bgClass = ot > 0 ? (ot >= 60 ? 'tw-bg-orange-100' : 'tw-bg-amber-50') : '';
        return (
  ...
  const allCols = useMemo<ColumnsType<OvertimeUsage>>(
    () => [...cols, ...bucketCols],
    [cols, bucketCols],
  );
        ...
        <div className="tw-flex tw-flex-wrap tw-items-center tw-gap-x-4 tw-gap-y-3">
          ...
          <Space.Compact>
            ...
            <Button
              icon={<RightOutlined />}
              onClick={() =>
                setBaseDate((d) => (periodMode === 'WEEK' ? d.add(1, 'week') : d.add(1, 'month')))
              }
          ...
          emptyText: (
            <div className="tw-py-6 tw-text-slate-500">
              ...
              <div className="tw-mt-1 tw-text-xs">
                선택 시점({iso})에 사용률 {Math.max(50, thresholdPct)}% 이상 직원이 없거나, 검색
                조건에 맞지 않습니다.
              </div>
            </div>
          ),
    ...
    <Space direction="vertical" className="tw-w-full" size={16}>
      <Alert
        ...
        description={
          readOnly ? null : (
            <span className="tw-text-xs">
              홈택스 -&gt; 세무업무별 서비스 -&gt; 원천징수 -&gt; 근로소득간이세액표 에서 엑셀
              다운로드 후 업로드하세요. 같은 연도 재업로드 시 기존 행은 자동 갱신됩니다.
            </span>
          )
        }
      />
...
export function AdminSalarySettingsPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const tabItems = useMemo(
    () => [
        ...
        ? [
            {
              key: 'pay-grade-table',
              label: '호봉표 관리',
              children: <AdminPayGradeTablePage embedded />,
            },
          ]
        : []),
      ...
      { key: 'tax', label: '세율 (조회)', children: <TaxRateTab readOnly /> },
      {
        key: 'simplified-tax',
        label: '간이세액표 (조회)',
        children: <SimplifiedTaxTableTab readOnly />,
      },
    ],
    [hasPayGradePolicy],
  );
