  ...
  const alignedObjective = useMemo(() => {
    ...
    return (
      availableObjectives.find((item) => item.goalId === goal.alignedOrgGoalId) ?? {
        ...
        title: goal.objectiveTitle ?? '연결된 조직 목표',
    ...
    goal.objectiveGradeA,
    goal.objectiveGradeB,
    goal.objectiveGradeC,
    goal.objectiveGradeS,
    goal.objectiveTitle,
    isObjective,
  ]);
          ...
          {!compact && isObjective ? (
            ...
            <div className="tw-mt-3 tw-space-y-2">
              <div className="tw-flex tw-items-center tw-gap-1.5 tw-text-[11px] tw-text-slate-500">
                <span className="tw-font-semibold tw-text-slate-400">상위 조직 목표</span>
            ...
            <>
              ...
              {!compact && aggregate && (
                <div className="tw-mt-0.5 tw-text-[10px] tw-text-slate-400">
                  확정 개인 목표 {aggregate.confirmedCount} / 전체 {aggregate.childCount}
                </div>
              )}
            </>
          ) : (
