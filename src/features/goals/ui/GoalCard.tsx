      ...
      <div className="tw-flex tw-items-start tw-justify-between tw-gap-4">
        <div className="tw-min-w-0 tw-flex-1">
          <div className="tw-mb-2 tw-flex tw-flex-wrap tw-items-center tw-gap-1.5">
            ...
            >
              {isObjective ? '조직 목표' : '개인 목표'}
            </Tag>
            <Tag color={STATUS_COLOR[goal.status] as any} className="!tw-m-0 !tw-rounded-full">
              {STATUS_LABEL[goal.status]}
            </Tag>
            <Tag bordered={false} className="!tw-m-0 !tw-rounded-full !tw-bg-slate-100 !tw-text-slate-700">
              {goal.cycleKey || goal.cycle}
            </Tag>
            {showOwnerName && goal.ownerType === 'MEMBER' ? (
              <Tag bordered={false} className="!tw-m-0 !tw-rounded-full !tw-bg-purple-50 !tw-text-purple-700">
          ...
          {!compact ? (
            <div className="tw-mt-3 tw-grid tw-grid-cols-2 tw-gap-2 md:tw-grid-cols-4">
              {(['S', 'A', 'B', 'C'] as const).map((grade) => {
                const text =
                  grade === 'S' ? goal.gradeS : grade === 'A' ? goal.gradeA : grade === 'B' ? goal.gradeB : goal.gradeC;
                return (
        ...
        {!isObjective ? (
          <div className="tw-w-[72px] tw-shrink-0 tw-text-right">
            <div className="tw-text-[24px] tw-font-bold tw-leading-none tw-text-[#1e3a5f]">
              {goal.weightPct}
