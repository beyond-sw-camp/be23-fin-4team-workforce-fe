...
export function CalibrationForm({ response, currentUserId }: Props) {
  ...
  if (!myCal) {
    return <AppEmptyIllustrated description="이 응답의 calibration 권한이 없습니다." />;
  }
    ...
    <div className="tw-flex tw-flex-col tw-gap-5">
        ...
        <div className="tw-flex tw-items-center tw-justify-between tw-gap-4">
          <div>
            ...
            <div className="tw-mt-1 tw-text-xs tw-text-slate-500">
              {isLead
                ? 'Lead는 최종 등급을, Assistant는 제안 등급을 입력합니다. 기준 텍스트는 상위 Objective rubric입니다.'
      ...
      {snapshot.goals.map((goal) => {
        ...
        return (
          ...
          >
            <div className="tw-mb-3 text-sm tw-text-slate-500">{goal.description}</div>
            ...
            <div className="tw-mb-3 tw-flex tw-flex-wrap tw-items-center tw-gap-2">
              ...
              <Tag
                bordered={false}
                className="!tw-m-0 !tw-rounded-full !tw-bg-emerald-50 !tw-px-2.5 !tw-text-[11px] !tw-font-medium !tw-text-emerald-700"
              >
                자기평가 {selfGrade ?? '-'}
              </Tag>
      ...
      <Card
        className={SECTION_CARD}
        styles={{ body: { padding: 20 } }}
        title={<span className="tw-text-[15px] tw-font-semibold tw-text-slate-900">평가자 코멘트</span>}
      >
